...
const UseCaseSection = memo(() => {
  return (
    <section aria-labelledby="usecase-heading" className="bg-[#efeeea] dark:bg-[#080d10] py-16">
      <div className="px-4 mx-auto max-w-7xl">
        <h2 id="usecase-heading" className="mb-12 text-3xl font-extrabold md:text-4xl text-center text-black dark:text-white">
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {useCases.map((item) => (
            <Link
              key={item.title}
              to={item.path}
              aria-label={`Shop ${item.title} — ${item.desc}`}
